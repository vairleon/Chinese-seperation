...
import optimization
import tokenization
import tensorflow as tf    #(源码基于tensorflow)  -亦可见pytorch版 
import pickle
from evaluation import SegmenterEvaluation

flags = tf.flags   #用于帮助我们添加命令行的可选参数。 利用该函数我们可以实现在命令行中选择需要设定的参数来运行程序   eg: python3 run_cut.py --data_dir="/content" 
...
class InputFeatures(object):
    """A single set of features of data."""    #数据特征集
...
class DataProcessor(object):
    ...
    def _read_data(cls, input_file):
        ...
        def seg_long_sentence(words, labels):   #闭包   #分长句 

            def tough_seg(words, labels):   
                words_list = []
                labels_list = []
                while len(words) >  FLAGS.max_seq_length-2:     #将比最大序列长度更长的序列（句子）分割开/ 分成多个句子  word_list为其返回值
...
class CutProcessor(DataProcessor):      #分词处理
    def get_train_examples(self, data_dir):
    ...
    def _create_example(self, lines, set_type):
        ...
        return examples     # examples是一个InputExample实例列表  每个实例包含guid text lable(InputExample类中的属性)


def write_tokens(tokens, mode):    #生成 token_test.txt 文件 （无标记的原始语料）
    if mode == "test":
...
def output_seg_result(output_dir, ori_labels, des_labels):      #输出文件配置 输入：路径原始标记，待验证标记（BERT模型做的标记）
    token_file_path = os.sep.join([output_dir, "token_test.txt"]) #使用无标记原始语料   #xxx_lables 是整个文本的标记列表，每个列表元素是一行序列的标记
    with open(token_file_path, "r", encoding="utf-8") as f:
        tokens = [each.strip() for each in f.readlines()]
    lenth = len(tokens)
    assert len(ori_labels) == lenth  # assert的作用,若出错则知道是哪里出了问题,协助debug
    ...
    for i in range(lenth):
        ...
        des_seg = " ".join([token[each[0]:each[1]] for each in des_label])  #str.join()  以str将列表里的数据连接起来
        writer.write(token + "\n")
    ...
    writer.close()
####
def output_seg_result_train_ver(output_dir, des_labels): #自定义方法  - modify by ZYF, 新加一个输出文件 回喂给训练集
    token_file_path = os.sep.join([output_dir, "token_test.txt"])
    with open(token_file_path, "r", encoding="utf-8") as f:
        tokens = [each.strip() for each in f.readlines()]
    lenth = len(tokens)
    assert len(des_labels) == lenth
    seg_result_file = os.path.join(output_dir, "seg_result_train.txt")
    writer = open(seg_result_file, "w", encoding="utf-8")
    for i in range(lenth):
        token = tokens[i]
        des_label = des_labels[i]
        label = ""
        for each in des_label:   
            if each[1] - each[0] == 1:
                label = label + 's'
            else :
                label = label + 'b' + 'm' * (each[1] - each[0] - 2) + 'e'
        #assert len(token) != des_label[-1][1]      
...
def convert_single_example(ex_index, example, label_list, max_seq_length, tokenizer,mode):      ######################## important
    label_map = {}
...
def main(_):
    ...
    processors = {
        "people": CutProcessor          #people : task name
    }
    ...
    if FLAGS.do_predict:
        ...
        output_seg_result_train_ver(FLAGS.output_dir, des_labels)
